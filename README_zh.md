...
SkillFlow 是一款跨平台的智能体SKILL管理神器。它打破了不同智能体环境的藩篱，让提示词与技能随处复用。集本地优先的 Skill 管理、多端同步、仓库发现、实时更新与云端归档于一体，为你重塑高效、纯净的开发体验。
...
- **🎛️ 透明掌控**：
- 拒绝技能管理的“黑盒”状态。对海量技能按场景分类归档，确保开发者对智能体能力边界拥有完全知情权与控制权。
...
| **更新检测** | 检查 Git 来源 Skill 是否有新提交，可在仓库刷新后自动更新匹配的“我的skills”，并在应用内更新已安装副本以及已经推送到智能体目录的对应副本；当前勾选了自动同步的智能体会被自动覆盖为最新版本，未勾选的智能体仍保留手动更新路径。 |
| **桌面体验** | 支持中英文界面、多主题且默认使用 Young、helper 承载的托盘/菜单栏重开、开机自启、按智能体配置，以及窗口重新显示后立即延续当前页面内容与交互。 |
