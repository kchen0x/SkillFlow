...
| **云端备份** | 将 Skill 库镜像至阿里云 OSS、腾讯云 COS、华为云 OBS 或任意 Git 仓库，支持自定义对象存储远程路径预览、按服务商独立保存配置、敏感云凭据仅保存在本地，以及 Git 冲突时的手动处理入口 |
...
| **设置** | 按工具独立配置启用状态、推送/扫描路径、自定义工具、代理设置、可调的本地/远程扫描深度、按页面控制的卡片状态显示、不会参与同步的本地路径配置，以及设置页里的 `Ctrl+S` / `Cmd+S` 保存快捷键；目录选择器会优先回到当前路径 |
