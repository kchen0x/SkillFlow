...
| **GitHub 安装** | 克隆任意仓库，递归发现仓库内嵌套的 Skill 候选项，并一键选择安装；“已安装”判断按规范化仓库来源 + 子路径完成，因此同名候选项也能区分 |
| **跨工具同步** | 推送或拉取 Skills 到/从 Claude Code、OpenCode、Codex、Gemini CLI、OpenClaw 及自定义工具；候选 Skill 的已导入 / 已推送状态由后端统一解析，冲突按具体 skill-target 配对精确处理，不再依赖仅按名称的启发式判断 |
| **仓库收藏** | 关注 Git 仓库，无需导入即可递归浏览和使用仓库内嵌套的 Skills；“已导入”徽章按规范化仓库来源 + 子路径解析，可更新的已安装实例状态也会直接显示在仓库 Skill 卡片上；内置起始仓库（`anthropics/skills`、`ComposioHQ/awesome-claude-skills`、`affaan-m/everything-claude-code`）仅在首次初始化时注入，用户删除后不会在后续启动被自动加回 |
