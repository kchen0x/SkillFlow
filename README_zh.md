...
| **更新检测** | 检查 GitHub 来源 Skill 是否有新提交，并在应用内更新已安装副本以及已经推送到工具目录的对应副本；同时会覆盖同步当前勾选“自动推送目标工具”的副本，保持选中工具中的 Skill 为最新版本。 |
| **桌面体验** | 支持中英文界面、多主题、helper 承载的托盘/菜单栏重开、开机自启、按工具配置，以及窗口隐藏且长时间不活跃时的后台页面内存收缩。 |
