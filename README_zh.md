...
| **云端备份** | 将 Skill 库镜像至阿里云 OSS、腾讯云 COS、华为云 OBS 或任意 Git 仓库；所有服务商的备份范围统一为应用数据目录（排除 `cache/` 与 `.git/`）；Git 模式支持仓库/`origin` 自动初始化，并在冲突时展示冲突文件供选择本地或远端覆盖 |
| **更新检测** | 自动检测 GitHub 来源 Skills 的新提交；一键更新 |
| **应用自动更新** | 启动时横幅提示新版本；Windows 支持一键下载并重启；macOS 链接至 GitHub Releases 页面 |
