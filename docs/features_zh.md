...
- macOS 托盘行为：应用驻留在顶部状态栏（`SF` 图标）；使用原生单击即可展开菜单，可执行 `Show Window`、`Hide Window` 和 `Quit SkillFlow`。
...
- **右键上下文菜单**（每个分类）：
  ...
  - **删除** — 调用 `DeleteCategory()`；该分类下的 Skills 移至默认分类。（`Default` 分类不可用）
...
- 采用与"我的 Skills"接近的双栏布局。
- 左侧边栏显示分类筛选：**全部** 加所有现有分类。
- 右侧显示目标工具选择、推送模式按钮，以及当前分类范围内的 Skill 卡片网格。
